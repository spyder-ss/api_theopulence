...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Cities' }} Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage cities</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/cities/add') }}" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($model_data_lists) && $model_data_lists->count() > 0)
                            @foreach($model_data_lists as $city)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $city->status }}" data-title="{{ $city->name }}">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div class="h-10 w-10 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center">
                                                    <span class="text-sm font-medium text-white">{{ substr($city->name, 0, 1) }}</span>
                                                </div>
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $city->name }}</div>
                                                <div class="text-xs text-gray-500">City ID: {{ $city->id }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        <div class="flex items-center">
                                            <div class="text-sm font-medium text-gray-900">{{ $city->stateDetails ? $city->stateDetails->name : 'N/A' }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        <div class="flex items-center">
                                            <div class="text-sm font-medium text-gray-900">{{ $city->countryDetails ? $city->countryDetails->name : 'N/A' }}</div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        {{ \Carbon\Carbon::parse($city->created_at)->format('M d, Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/cities/add?id=' . $city->id) }}"
                                            ...
                                            @if($city->is_delete == 0)
                                                <form id="deleteForm{{ $city->id }}" method="POST" action="{{ url(getAdminRouteName() . '/cities/delete/' . $city->id) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                           onclick="confirmDelete('{{ url(getAdminRouteName() . '/cities/delete/' . $city->id) }}', '{{ $city->name }}')"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-500 text-lg font-medium">No cities found</p>
                                        <p class="text-gray-400 text-sm">Get started by creating your first city.</p>
                                        <a href="{{ url(getAdminRouteName() . '/cities/add') }}" class="mt-4 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add City</span>
    ...
    <script>
        function confirmDelete(url, cityName) {
            document.getElementById('deleteForm').action = url;
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the city "${cityName}"? This action cannot be undone.`;
        ...
        document.getElementById('statusFilter').addEventListener('change', filterCities);
