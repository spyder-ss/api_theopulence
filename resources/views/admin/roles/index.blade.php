...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Roles' }} Management</h1>
                        <p class="text-gray-600 text-lg">Manage user roles and their permissions across the system</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/roles/add') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_roles_for_stats) ? $all_roles_for_stats->where('status', 'Active')->count() : 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-600">Inactive Roles</p>
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_roles_for_stats) ? $all_roles_for_stats->where('status', 'Inactive')->count() : 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_roles_for_stats) ? $all_roles_for_stats->count() : 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($roles) && $roles->count() > 0)
                            @foreach($roles as $role)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $role->status }}" data-name="{{ $role->name }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-sm text-gray-500 md:hidden">{{ $role->role_for ?? 'N/A' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                        <span class="text-sm text-gray-900">{{ $role->role_for ?? 'N/A' }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/roles/edit/' . $role->id) }}"
                                            ...
                                            @if($role->is_delete != 1)
                                                <button type="button"
                                                       onclick="confirmDelete('{{ url(getAdminRouteName() . '/roles/delete/' . $role->id) }}', '{{ $role->name }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-400 text-sm">Get started by creating your first role.</p>
                                        <a href="{{ url(getAdminRouteName() . '/roles/add') }}" class="mt-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add New Role</span>
    ...
    <script>
        function confirmDelete(url, roleName) {
            ...
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the role "${roleName}"? This action cannot be undone.`;
