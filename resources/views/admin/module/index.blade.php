...
@section('content')
                ...
                <div class="flex items-center space-x-4">
                    <div class="p-3 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg">
                        <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14-8H5a2 2 0 00-2 2v10a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2z"/>
                        </svg>
                    </div>
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Modules' }} Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage system modules</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/modules/add') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($modules) && $modules->count() > 0)
                            @foreach($modules as $module)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $module->status }}" data-title="{{ $module->name }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        {{ \Carbon\Carbon::parse($module->created_at)->format('M d, Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/modules/add?id=' . $module->id) }}"
                                            ...
                                            @if($module->is_delete == 0)
                                                <form id="deleteForm{{ $module->id }}" method="POST" action="{{ url(getAdminRouteName() . '/modules/delete/' . $module->id) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                           onclick="confirmDelete('{{ url(getAdminRouteName() . '/modules/delete/' . $module->id) }}', '{{ $module->name }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-500 text-lg font-medium">No modules found</p>
                                        <p class="text-gray-400 text-sm">Get started by creating your first module.</p>
                                        <a href="{{ url(getAdminRouteName() . '/modules/add') }}" class="mt-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add Module</span>
    ...
    <script>
        function confirmDelete(url, moduleName) {
            ...
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the module "${moduleName}"? This action cannot be undone.`;
