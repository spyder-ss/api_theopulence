...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Amenities' }} Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage amenities</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/amenities/add') }}" class="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                ...
                <div class="flex flex-col lg:flex-row gap-4 justify-between items-center">
                    <div class="flex-1 flex items-center gap-4">
                        <div class="relative flex-1 max-w-md">
                            <input type="text" id="amenitySearch" placeholder="Search amenities..."
                        ...
                        <select id="statusFilter" class="border border-gray-300 rounded-lg text-sm px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors duration-200">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($model_data_lists) && $model_data_lists->count() > 0)
                            @foreach($model_data_lists as $amenity)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $amenity->status }}" data-name="{{ $amenity->name }}">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div class="h-10 w-10 rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center">
                                                    <span class="text-sm font-medium text-white">{{ substr($amenity->name, 0, 1) }}</span>
                                                </div>
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/amenities/add?id=' . $amenity->id) }}"
                                            ...
                                            @if($amenity->is_delete == 0)
                                                <form id="deleteForm{{ $amenity->id }}" method="POST" action="{{ url(getAdminRouteName() . '/amenities/delete/' . $amenity->id) }}" class="inline">
                                                    @csrf
                                                    @method('POST') {{-- Changed to POST for consistency with BlogController --}}
                                                    <button type="button"
                                                           onclick="confirmDelete('{{ url(getAdminRouteName() . '/amenities/delete/' . $amenity->id) }}', '{{ $amenity->name }}')"
                        ...
                        @else
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center"> {{-- Changed colspan from 4 to 3 --}}
                                    <div class="flex flex-col items-center space-y-3">
                                        <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        </svg>
                                        <p class="text-gray-500 text-lg font-medium">No amenities found</p>
                                        <p class="text-gray-400 text-sm">Get started by creating your first amenity.</p>
                                        <a href="{{ url(getAdminRouteName() . '/amenities/add') }}" class="mt-4 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add Amenity</span>
    ...
    <script>
        function confirmDelete(url, amenityName) {
            document.getElementById('deleteForm').action = url;
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the amenity "${amenityName}"? This action cannot be undone.`;
        ...
        document.getElementById('amenitySearch').addEventListener('input', filterSettings);
