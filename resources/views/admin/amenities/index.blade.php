...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($title) ? $title : 'Amenities' }} Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage amenities</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/amenities/create') }}" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($amenities) && $amenities->count() > 0)
                            @foreach($amenities as $amenity)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $amenity->status }}" data-title="{{ $amenity->name }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-xs text-gray-500">Amenity ID: {{ $amenity->id }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        {{ \Carbon\Carbon::parse($amenity->created_at)->format('M d, Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/amenities/' . $amenity->id . '/edit') }}"
                                            ...
                                            <form id="deleteForm{{ $amenity->id }}" method="POST" action="{{ url(getAdminRouteName() . '/amenities/' . $amenity->id) }}" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                                       onclick="confirmDelete('{{ url(getAdminRouteName() . '/amenities/' . $amenity->id) }}', '{{ $amenity->name }}')"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <a href="{{ url(getAdminRouteName() . '/amenities/create') }}" class="mt-4 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add Amenity</span>
    ...
    <script>
        ...
        document.getElementById('amenitySearch').addEventListener('input', filterAmenities);
        document.getElementById('statusFilter').addEventListener('change', filterAmenities);
