...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Blogs' }} Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage blog posts</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/blogs/add') }}" class="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($model_data_lists) && $model_data_lists->count() > 0)
                            @foreach($model_data_lists as $blog)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $blog->status }}" data-title="{{ $blog->title }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-xs text-gray-500">{{ $blog->author }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 max-w-xs">
                                        <div class="truncate" title="{{ $blog->brief }}">
                                            @if(strlen($blog->brief) > 50)
                                                {{ substr($blog->brief, 0, 50) }}...
                                            @else
                                                {{ $blog->brief }}
                                            @endif
                                        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell">
                                        {{ \Carbon\Carbon::parse($blog->posted_on)->format('M d, Y') }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/blogs/add?id=' . $blog->id) }}"
                                            ...
                                            @if($blog->is_delete == 0)
                                                <form id="deleteForm{{ $blog->id }}" method="POST" action="{{ url(getAdminRouteName() . '/blogs/delete/' . $blog->id) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                           onclick="confirmDelete('{{ url(getAdminRouteName() . '/blogs/delete/' . $blog->id) }}', '{{ $blog->title }}')"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-400 text-sm">Get started by creating your first blog post.</p>
                                        <a href="{{ url(getAdminRouteName() . '/blogs/add') }}" class="mt-4 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add Blog</span>
    ...
    <script>
        function confirmDelete(url, blogTitle) {
            document.getElementById('deleteForm').action = url;
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the blog post "${blogTitle}"? This action cannot be undone.`;
        ...
        document.getElementById('blogSearch').addEventListener('input', filterSettings);
