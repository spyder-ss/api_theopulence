...
@section('content')
    <div class="flex-1 p-6 overflow-auto bg-gray-50">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ $module_name ?? 'Blog Management' }}</h1>
                        <p class="text-gray-600 text-lg">Create and manage blog posts</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/blogs') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            <form method="POST" action="{{ url(getAdminRouteName() . '/blogs/add' . (isset($model_data->id) ? '?id=' . $model_data->id : '')) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   id="author"
                                   name="author"
                                   value="{{ old('author', $model_data->author ?? '') }}"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors duration-200 @error('author') border-red-500 @enderror"
                        ...
                        <!-- Posted On -->
                        <div>
                            <label for="posted_on" class="block text-sm font-medium text-gray-700 mb-2">
                                Posted On
                            </label>
                            <input type="date"
                                   id="posted_on"
                                   name="posted_on"
                                   value="{{ old('posted_on', isset($model_data->posted_on) ? date('Y-m-d', strtotime($model_data->posted_on)) : date('Y-m-d')) }}"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors duration-200 @error('posted_on') border-red-500 @enderror">
                            @error('posted_on')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="url"
                                   id="link"
                                   name="link"
                                   value="{{ old('link', $model_data->link ?? '') }}"
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <textarea id="brief"
                                      ...
                                      placeholder="Brief description of the blog post">{{ old('brief', $model_data->brief ?? '') }}</textarea>
                            @error('brief')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <textarea id="description"
                                      name="description"
                                      rows="6"
                                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors duration-200 ckeditor @error('description') border-red-500 @enderror"
                                      placeholder="Full description of the blog post">{{ old('description', $model_data->description ?? '') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            @if(isset($model_data->image) && $model_data->image)
                                <div class="mt-2 flex items-center space-x-2">
                                    <img src="{{ asset('storage/blog/' . $model_data->id . '/' . $model_data->image) }}" alt="Current Image" class="w-20 h-20 object-cover rounded">
                ...
                <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                    ...
                    <button type="submit"
                        ...
                        {{ isset($model_data->id) ? 'Update Blog' : 'Create Blog' }}
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        function deleteImage(id, type) {
            if (confirm('Are you sure you want to delete this image?')) {
                let url = '{{ url(getAdminRouteName() . "/blogs/ajax_img_delete") }}';
