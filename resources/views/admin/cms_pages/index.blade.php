...
@section('content')
    <div class="flex-1 p-6 overflow-auto bg-gray-50">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'CMS Pages' }} Management</h1>
                        <p class="text-gray-600 text-lg">Manage your website pages and content</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/cms_pages/add') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
        ...
        @if(request()->parent_id)
            <div class="mb-6">
                <a href="{{ url(getAdminRouteName() . '/cms_pages') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700 hover:text-gray-900 transition-colors duration-200">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($model_data_lists) && $model_data_lists->count() > 0)
                            @foreach($model_data_lists as $page)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $page->status }}" data-name="{{ $page->name }}" data-template="{{ $page->template }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            @if($page->image)
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    <img src="{{ asset('storage/cms_page/' . $page->id . '/' . $page->image) }}" alt="{{ $page->name }}" class="h-10 w-10 rounded-lg object-cover">
                                                </div>
                                            @else
                                                <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-400 to-blue-600 rounded-lg flex items-center justify-center">
                                                    <span class="text-sm font-medium text-white">{{ substr($page->name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $page->name }}</div>
                                                <div class="text-sm text-gray-500">Slug: {{ $page->slug }}</div>
                                                @if($page->getChildPage->count() > 0)
                                                    <div class="mt-1">
                                                        <a href="{{ url(getAdminRouteName() . '/cms_pages') }}?parent_id={{ $page->id }}" class="inline-flex items-center text-xs text-blue-600 hover:text-blue-800">
                                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                                            </svg>
                                                            {{ $page->getChildPage->count() }} sub-pages
                                                        </a>
                                                    </div>
                                                @endif
                                            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden lg:table-cell max-w-xs">
                                        <div class="truncate" title="{{ $page->brief }}">
                                            @if($page->brief)
                                                @if(strlen($page->brief) > 50)
                                                    {{ substr($page->brief, 0, 50) }}...
                                                @else
                                                    {{ $page->brief }}
                                                @endif
                                            @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ url(getAdminRouteName() . '/cms_pages/add?id=' . $page->id) }}"
                                            ...
                                            @if($page->is_delete == 0)
                                                <form id="deleteForm{{ $page->id }}" method="POST" action="{{ url(getAdminRouteName() . '/cms_pages/delete/' . $page->id) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                           onclick="confirmDelete('{{ url(getAdminRouteName() . '/cms_pages/delete/' . $page->id) }}', '{{ $page->name }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-500 text-lg font-medium">No pages found</p>
                                        <p class="text-gray-400 text-sm">Get started by adding your first page.</p>
                                        <a href="{{ url(getAdminRouteName() . '/cms_pages/add') }}" class="mt-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add Page</span>
    ...
    <script>
        function confirmDelete(url, pageName) {
            document.getElementById('deleteForm').action = url;
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the page "${pageName}"? This action cannot be undone.`;
        ...
        function filterPages() {
            ...
            rows.forEach(row => {
                ...
                const template = row.getAttribute('data-template').toLowerCase();
                const status = row.getAttribute('data-status');
                const textContent = row.textContent.toLowerCase();

                const matchesSearch = textContent.includes(searchTerm) || name.includes(searchTerm) || template.includes(searchTerm);
