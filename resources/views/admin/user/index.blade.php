...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ isset($module_name) ? $module_name : 'Users' }} Management</h1>
                        <p class="text-gray-600 text-lg">Manage system users and their account details</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ url(getAdminRouteName() . '/users/add') }}" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                        </svg>
                        <span>Add New User</span>
                    </a>
                    {{--<a href="{{ url(getAdminRouteName() . '/users/customers') }}" class="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_users_for_stats) ? $all_users_for_stats->where('status', 1)->count() : 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_users_for_stats) ? $all_users_for_stats->where('status', 0)->count() : 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ isset($all_users_for_stats) ? $all_users_for_stats->count() : 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <div class="flex flex-col lg:flex-row gap-4 justify-between items-center">
                    <div class="flex-1 flex items-center gap-4">
                        ...
                        <select id="roleFilter" class="border border-gray-300 rounded-lg text-sm px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if(isset($users) && $users->count() > 0)
                            @foreach($users as $user)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $user->status }}" data-role="{{ $user->role_id }}" data-name="{{ $user->name }}">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-400 text-sm">Get started by creating your first system user.</p>
                                        <a href="{{ url(getAdminRouteName() . '/users/add') }}" class="mt-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add New User</span>
    ...
    <script>
        function confirmDelete(url, userName) {
            ...
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the user "${userName}"? This action cannot be undone.`;
        ...
        document.getElementById('clearFilters').addEventListener('click', filterFilters);
