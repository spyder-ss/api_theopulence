        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('admin.common-image-categories.add') }}"
                        class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                ...
                <div class="flex flex-col lg:flex-row gap-4 justify-between items-center">
                    <div class="flex-1 flex items-center gap-4">
                        ...
                        <select id="statusFilter"
                            class="border border-gray-300 rounded-lg text-sm px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-colors duration-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @if (isset($model_data_lists) && $model_data_lists->count() > 0)
                            @foreach ($model_data_lists as $category)
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $category->status }}"
                                            ...
                                            <div class="flex-shrink-0 h-10 w-10">
                                                @if ($category->image)
                                                    <img class="h-10 w-10 rounded-full object-cover"
                                                        src="{{ asset('storage/common_image_categories/' . $category->image) }}"
                                                        alt="{{ $category->name }}">
                                                @else
                                                    <div
                                                        class="h-10 w-10 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center">
                                                        <span
                                                            class="text-sm font-medium text-white">{{ substr($category->name, 0, 1) }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <form id="deleteForm{{ $category->id }}" method="POST"
                                                action="{{ route('admin.common-image-categories.delete', $category->id) }}"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-400 text-sm">Get started by adding your first common image
                                            category.</p>
                                        <a href="{{ route('admin.common-image-categories.add') }}"
                                            class="mt-4 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                            </svg>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500" id="modalMessage">
                                    Are you sure you want to delete this common image category? This action cannot be
                                    undone.
                                </p>
                            </div>
                        </div>
    ...
    <script>
        function confirmDelete(url, categoryName) {
            ...
            document.getElementById('modalMessage').innerHTML =
                `Are you sure you want to delete the common image category "${categoryName}"? This action cannot be undone.`;
        ...
        function filterCategories() {
            ...
            rows.forEach(row => {
                ...
                const slug = row.querySelector('td:nth-child(2) > div').textContent
                    .toLowerCase(); // Assuming slug is the second column now
