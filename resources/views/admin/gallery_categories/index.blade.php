...
@section('content')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-1">Gallery Categories Management</h1>
                        <p class="text-gray-600 text-lg">Create and manage gallery categories</p>
                    </div>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('admin.gallery-categories.add') }}" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150" data-status="{{ $category->status }}" data-name="{{ $category->name }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <form id="deleteForm{{ $category->id }}" method="POST" action="{{ route('admin.gallery-categories.delete', $category->id) }}" class="inline">
                                                @csrf
                                                @method('POST') {{-- Changed to POST as per route definition --}}
                                                <button type="button"
                                                        onclick="confirmDelete('{{ route('admin.gallery-categories.delete', $category->id) }}', '{{ $category->name }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <p class="text-gray-500 text-lg font-medium">No gallery categories found</p>
                                        <p class="text-gray-400 text-sm">Get started by adding your first gallery category.</p>
                                        <a href="{{ route('admin.gallery-categories.add') }}" class="mt-4 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
                                            <span>Add New Category</span>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500" id="modalMessage">
                                    Are you sure you want to delete this gallery category? This action cannot be undone.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function confirmDelete(url, categoryName) {
            ...
            document.getElementById('modalMessage').innerHTML = `Are you sure you want to delete the gallery category "${categoryName}"? This action cannot be undone.`;
